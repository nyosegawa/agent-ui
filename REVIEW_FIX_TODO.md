...
- [ ] [F01] Fix hidden-source / pinned-source approval arrival so the required decision is visible or reachable when follow-scroll is at the transcript tail.
...
- [ ] [F02] Align stable `thread/resume` runtime behavior and docs with the stable-vs-raw policy.
...
- [ ] [F03] Update `app/list` normalization and current fixtures to current generated `AppInfo` vocabulary.
...
- [ ] [F04] Resolve the raw public `ServerRequestQueueState` key contract.
...
- [ ] [F05] Fix `FakeAgentTransport.close()` so extra same-generation waiters do not hang.
...
- [ ] [F06] Make one-shot default method enforcement private and immutable.
...
- [ ] [F07] Redact all plain-text `Authorization:` header values, not only Bearer tokens.
...
- [ ] [F08] Add `@nyosegawa/agent-ui-codex/normalizer` to package export docs.
...
- [ ] [F09] Correct validation evidence and docs-sync overclaims.
...
- [ ] [F10] Make the dark/system token parity guard media-aware.
...
- [ ] [F11] Handle rejecting Next sidecar upload cleanup without unhandled or cached rejections.
...
- [ ] [F12] Redact credential query parameters whose names are percent-encoded.
...
- [ ] [F13] Decide and enforce or document dynamic helper permission callback value broadening.
...
- [ ] [F14] Strengthen method-result exactness beyond method-key coverage.
...
- [ ] [V01] Validate React approval and style fixes with focused local gates.
...
- [ ] [V02] Validate Codex protocol, request-builder, app normalizer, fixture, and method-result fixes with targeted Codex gates.
...
- [ ] [V03] Validate core state and fake transport fixes with focused core tests.
...
- [ ] [V04] Validate server security, bridge policy, dynamic-tools, and example cleanup fixes with focused server tests.
...
- [ ] [V05] Validate docs, package export lists, and API snapshots only where the public surface changed.
