...
`useAgentThreadHistory().listThreads()` calls `thread/list`, supports search and pagination cursor inputs, tracks the latest returned cursor, and upserts returned thread metadata without activating it. The default `ThreadSidebar` uses the same hook for startup history loading, `Load more` pagination, and then calls `useAgentThreadReader()` to preview the newest stored session when no thread is active.
