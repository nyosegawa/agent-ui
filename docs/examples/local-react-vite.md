...
file size. Layout reachability lives in `visual-layout.e2e.ts`, close-up gallery
coverage lives in `visual-closeups.e2e.ts`, approval behavior lives in
`visual-approvals.e2e.ts`, and token-backed design-system invariants live in
`design-system-contract.e2e.ts`. Keep new fixture browser checks in the file
that owns the contract they protect, and put only thin reusable assertions in
`e2e/support/`.
