...
### 5. GitHub Actions は監視しない

- GitHub Actions は usage を使い切っているため、push 後も監視しない。
- `gh run list`、`gh run view`、`gh api .../actions/...`、GitHub Actions logs の確認は行わない。
- CI 結果の代わりに、この文書で指定した local focused validation と broad local gate を実行して報告する。
- ユーザーが明示的に GitHub Actions の確認を依頼した場合だけ、監視を再開する。

### 6. docs と tests を実装と同じ slice に含める
...
### 8. security default changes は breaking として扱う
...
- GitHub Actions は usage 切れのため監視していないこと。
