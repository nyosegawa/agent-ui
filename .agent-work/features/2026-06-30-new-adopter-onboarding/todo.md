...
- [x] P006 Success-path fake App Server fixture and adopter validation story
  ...
  - Evidence:
    ...
    - Implementation: Fixture RPC responses are constrained with generated stable App Server response types for `ThreadStartResponse`, `TurnStartResponse`, and `ThreadReadResponse`; emitted turn status uses App Server `inProgress` rather than host-only wording.
    - Implementation: Added package export map entry, tsup entry, API snapshot, runtime named-export policy, package resolution canonical surface entry, post-publish smoke specifier, package README docs, package exports reference, first-host-app smoke example, and public skill validation guidance.
    - Validation: `bunx vitest run packages/codex/test/test-fixtures.test.ts packages/codex/test/public-surface.test.ts test/runtime-export-policy.test.ts` passed.
    - Validation: `bun run --cwd packages/codex typecheck` passed.
    - Validation: `bun run test:api-snapshots:update && bun run test:api-snapshots` passed.
    - Validation: `bun run test:package-resolution` passed.
    - Validation: `bun run validate:packages` passed with existing non-failing publint repository URL suggestions.
    - Validation: `bun run test:skills` passed.
    - Validation: `bun run test:e2e:fixtures` passed: 231 passed, 1 skipped.
    - Validation: `bun run test:e2e:clean-ports && env -u NO_COLOR -u FORCE_COLOR bunx playwright test --config playwright.real-local.config.ts examples/codex-local-web/e2e/real-local-thread-lifecycle.e2e.ts` passed: 9 passed. An earlier direct `playwright` invocation failed because the binary was not on PATH; rerun with `bunx playwright` passed.
    - Validation: `bun run lint` passed with 4 existing `react-hooks/exhaustive-deps` warnings in `packages/react/src/hooks/composer.ts`.
    - Validation: `bun run test:protocol` passed: 13 files, 103 tests.
    - Validation: `git diff --check` passed.
  ...
  - Tasks:
    ...
    - [x] T019 Cover canonical id, `thread/start`, `turn/start`, streamed delta, `turn/completed`, queued steer, and interrupt behavior.
    ...
    - [x] T020 Add adopter-facing success smoke guidance without requiring repository-only commands.
