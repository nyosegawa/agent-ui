...
use `turn/steer`, while idle Cmd/Ctrl+Enter starts a normal turn; Stop uses only
`turn/interrupt`. Restored context usage from `thread/tokenUsage/updated` appears
beside the composer when nonzero. The root route stays on the start screen;
stored threads are opened only through explicit selection or `/threads/<id>`
direct links.
